// 2026-08-06 新增，2026-08-11 升級為主力，2026-08-12 確定為唯一正式來源：純資料+公式計算的
// 每日報告產生器，不依賴 Claude 雲端 AI routine（CCR）。
//
// 背景：CCR 在 8/5、8/6、8/7、8/10、8/11 反覆卡死沒有產出，官方文件本身也明講 CCR 屬於
// research preview、不建議用在關鍵自動化上。2026-08-12 使用者明確指出「等 CCR 恢復再覆蓋」
// 這種設計不合理——既然講好這支腳本是正式主力，就不該再留一條讓不可靠的 CCR 靜默蓋掉結果的
// 後路。因此：CCR 每日 08:30 自動排程（daily-trading-report-0830）已暫停，這份報告就是
// 每個交易日唯一、最終的正式版本，不會再有「等更好版本」的話術或覆蓋行為。
//
// 已查核的風險項目：處置股排除（官方 API，非 AI 判斷但同樣可信；注意股目前未查核，見下方函式
// 註解）、SOX 費半指數連動（半導體供應鏈候選股在 SOX 大跌時調降信心分數）。誠實的能力邊界寫在
// data_quality.warnings：這份報告沒有法人動向/個股新聞/法說會這類需要真人或 AI 閱讀理解的敘事
// 內容——如果需要這塊，要在對話中另外請人（我）現場查證產出，不是靠背景自動化默默補上。
const fs = require('fs');
...
async function main() {
  ...
  const baseRiskTag = (c) => {
    const parts = ['本檔為公式計算結果，已排除官方公告的處置股（注意股本版本未查核），未經法說會與個股新聞查證，僅供參考'];
  ...
  const warnings = [
    '這份報告由 GitHub Actions 純程式化產生，是每個交易日唯一、最終的正式版本（2026-08-12起，不再有 CCR 覆蓋機制），不依賴 Claude 雲端 AI（CCR）、也不依賴任何對話 session 存活，每個交易日 08:20 觸發。',
    ...
    '候選池來自前一交易日 TWSE 成交量/漲跌幅排行（GitHub Actions 抓取，真實資料），未經「是否有掛牌個股期貨」的資格確認，也沒有法人動向、個股新聞、法說會這類需要人工或 AI 閱讀理解的敘事內容——這是這套自動化目前的能力邊界，不是暫時性的，需要更深入分析要在對話中另外請人查證產出。'
  ];
